...
function installRequirements(targetFolder, serverless, options) {
  ...
  if (options.dockerizePip) {
    ...
    if (options.dockerSsh) {
      ...
      dockerCmd.push(
        '-v',
        `${process.env.HOME}/.ssh/id_rsa:/root/.ssh/id_rsa:ro`,
        '-v',
      ...
      // copyCmd = ['cp', '/root/tmp/id_rsa', '/root/.ssh/id_rsa'];
      // chmodCmd = ['chmod', '600', '/root/.ssh/id_rsa'];
      // pipCmds.unshift(chmodCmd);
      // pipCmds.unshift(copyCmd);
    }
  ...
  if (dockerCmd.length) {
    ...
    dockerCmd.push(...mergeCommands([['ssh', '-T', 'git@gitlab.com']]));
    mainCmds = [dockerCmd];
  } else {
    mainCmds = pipCmds;
  }
